...
@if ($paginator->onFirstPage())
    <a href="{{ $paginator->url(1) }}" rel="prev" class="pageHidden pagePadding"><<</a>
@else
    <a href="{{ $paginator->url(1) }}" rel="prev" class="pagePadding"><<</a>
@endif
...
@endphp
@if ($paginator->onFirstPage())
    <a href="{{ $paginator->url($previousBlockPage) }}" rel="prev" class="pageHidden pagePadding"><</a>
@else
    <a href="{{ $paginator->url($previousBlockPage) }}" rel="prev" class="pagePadding"><</a>
@endif
...
@if ($paginator->hasMorePages())
    <a href="{{ $paginator->url($nextBlockPage) }}" rel="next" class="pagePadding">></a>
@else
    <a href="{{ $paginator->url($nextBlockPage) }}" rel="next" class="pageHidden pagePadding">></a>
@endif

{{-- 마지막 페이지 --}}
@if ($paginator->hasMorePages())
    <a href="{{ $paginator->url($paginator->lastPage()) }}" rel="next" class="pagePadding">>></a>
@else
    <a href="{{ $paginator->url($paginator->lastPage()) }}" rel="next" class="pageHidden pagePadding">>></a>
@endif